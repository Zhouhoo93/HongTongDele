...
- (UIView *)titleBackgroundView{
    if (!_titleBackgroundView) {
        ...
        _titleBackgroundView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    }
...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor:RGBColor(63, 172, 255)
                         forState:UIControlStateNormal];
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn setTitle:@"完成"
                  forState:UIControlStateNormal];
        [_sureBtn setTitleColor:RGBColor(63, 172, 255)
                       forState:UIControlStateNormal];
...
- (void)loadAddressData{
    NSString * filePath = [[NSBundle mainBundle] pathForResource:@"json(1)"
                                                          ofType:@"txt"];
...
- (NSString *)pickerView:(UIPickerView *)pickerView
    ...
    if (0 == component) {
        ...
        //截取空格前的字符
        NSString *area = c.areas[row];
        NSArray *strarray = [area componentsSeparatedByString:@"-"];
        return strarray[0];
    }
    return nil;
}
